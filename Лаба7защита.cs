...
abstract class Discipline
{
    ...
    public void Print(Athlet result, int position)
    {
        Console.WriteLine($"{position}. {_namediscipline}, {result._surname}, {result.Bestresult} метров");
    }
    public void MergeSort(Athlet[] arr, int left, int right)
    {
        if (left < right)
        {
    ...
    private void Merge(Athlet[] arr, int left, int middle, int right)
    {
        int n1 = middle - left + 1;
        int n2 = right - middle;

        Athlet[] L = new Athlet[n1];
        Athlet[] R = new Athlet[n2];
        ...
        while (p < n1 && q < n2)
        {
            if (L[p].Bestresult >= R[q].Bestresult)
            {
                arr[k] = L[p];
                p++;
            }
...
class Program
{
    static void Main()
    {
        ...
        disciplines[0].MergeSort(results, 0, results.Length - 1);

        Console.WriteLine("Итоговый протокол: ");
        for (int i = 0; i < results.Length; i++)
        {
            disciplines[i % 2].Print(results[i], i + 1);
        }
    }
}
//1 уровень 1 задача
